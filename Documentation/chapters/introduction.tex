...
Organizations increasingly rely on data-driven decision making, hence operational systems like transactional databases, APIs and SaaS applications are continuously integrated into analytics platforms. Extract-Transform-Load(ETL) is a common way to do so, ETL pipelines ingest, clean, reconcile and publish data for downstream reporting, data analytics and machine learning. However, real-world data sources evolve schemas when requirements change, upstream systems are upgraded, or new attributes are introduced. Even small schema changes like renaming a column will invalidate existing transformations and break downstream dependencies.

The academic literature has long recognized schema evolution as a fundamental challenge in data integration. Early work on mapping adaptation studied how schema mappings (declarative specifications that relate a source schema to a target schema) should be maintained as schemas change \cite{velegrakis2003mapping,yu2005semantic}. These mapping-centric approaches are central to many integration paradigms and have been closely connected to schema matching research, which aims to identify correspondences between heterogeneous schemas \cite{rahm2001survey}. Despite decades of progress, practitioners still face substantial costs when pipelines break and often requires manual diagnosis, re-implementation of transformations, and extensive validation.
...
The thesis involves the following research streams: (i) schema evolution and mapping adaptation, (ii) ETL pipeline operation and maintenance, and (iii) LLM/RAG-based automation for generating transformation logic. The main objective is to reduce the manual effort required to keep ETL pipelines correct and operational as schemas evolve, while preserving the semantics embedded in the existing, validated mappings.
...
Therefore, the main problem addressed in this thesis is to \emph{Detect schema evolution events and regenerate mapping transformations in ETL pipelines in a way that preserves semantics and is operationally feasible, using evidence-grounded LLM techniques.} In particular, the work investigates constructing a retrieval corpus from historical schema versions, machine-readable change sets, and previously validated transformation code, and use this evidence to drive the regeneration of executable ETL mappings under schema evolution.
...
\textbf{RQ0:} How can retrieval-augmented large language models with detected schema evolution be applied to automatically regenerate mapping transformations in ETL pipelines while preserving data semantics and maintaining operational reliability?

\subsection{Sub-Questions}

\begin{itemize}
  \item \textbf{RQ1 (Detection):} What precision, recall, and F\(_1\) scores can the schema change detector achieve for added, removed, and renamed columns in the NYC taxi V1~\(\rightarrow\)~V2 evolution scenario when evaluated against ground-truth change sets?
  ...
  \item \textbf{RQ3 (Operational feasibility):} In the orchestrated ETL workflow, what success rate and time-to-adaptation (including the time reduction ratio between manual and AI-assisted conditions) are achieved when integrating detection and regeneration components, and how do these compare with purely manual repair?
\end{itemize}
...
Chapter~1 Introduces the research context, problem, aims, and questions.

Chapter~2 presents the basics and theoretical foundations, including schema evolution, schema matching and mapping, ETL orchestration, and retrieval-augmented language models. 

Chapter~3 reviews related work on schema evolution, mapping adaptation, schema-centric use of LLMs, and adaptive detection. 

Chapter~4 describes the methodological approach, including the system architecture, datasets, and evaluation design. 

Chapter~5 reports experimental results for schema change detection, canonical ingestion, and end-to-end AI-assisted workflows. 

Chapter~6 discusses these results in relation to the literature, articulates limitations, and draws out practical implications. Finally, Chapter~7 concludes the thesis with a concise summary of contributions and an outlook on future work.