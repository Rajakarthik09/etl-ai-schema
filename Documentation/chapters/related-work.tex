...
ReMatch~\cite{sheetrit2024rematch} takes a retrieval-enhanced approach to schema matching, constructing rich textual representations of tables and using LLMs together with information-retrieval components to propose correspondences. The system is designed to work without labelled training data or direct access to source database contents, and reports strong performance improvements over previous machine-learning-based matchers on large, real-world schemas. Magneto~\cite{magneto2024} addresses cost and accuracy trade-offs by combining small language models (SLMs) with LLMs in a two-phase pipeline: retrieval using efficient SLM-based strategies, followed by LLM-based reranking. The method includes self-supervised fine-tuning using LLM-generated synthetic training data and effective prompting strategies for reranking. Magneto achieves high accuracy across different domains while maintaining scalability, and introduces a new benchmark with real biomedical datasets.
...
Retrieval-augmented generation was introduced by Lewis et al.~\cite{lewis2020retrieval} as a way of coupling a standard sequence-to-sequence language model (the ``parametric'' component) with a separate neural retriever and document index (the ``non-parametric'' memory). At a high level, the model retrieves supporting passages from a large corpus such as Wikipedia and conditions its generation on this retrieved evidence, rather than relying only on information stored in its weights. The paper studies two concrete variants---one that fixes a shared set of passages for the whole output, and one that allows different tokens to attend to different retrieved passages---and shows that both variants outperform strong baselines on several knowledge-intensive question-answering benchmarks. This work provides the conceptual template for the RAG-style prompting used in this thesis, where schemas, change sets, and code snippets play the role of retrieved documents.

\subsection{RAG for Tabular Data Discovery}

Pneuma~\cite{pneuma2025} applies the RAG idea to the problem of finding and preparing tables scattered across databases and data lakes. It uses LLMs to build rich representations of tables and combines these with more traditional mechanisms such as keyword and vector search, so that users can issue natural-language queries and retrieve relevant tables. The system is evaluated on six diverse real-world collections spanning enterprise and scientific data, and reports higher accuracy and better resource efficiency than both conventional table search tools and previous RAG-style systems. Although Pneuma focuses on discovery rather than schema evolution, it illustrates how retrieved tabular artefacts can be used to ground LLM behaviour, a design pattern mirrored in this thesis.
...
%
%Robustness of text-to-SQL systems under schema evolution has been examined in recent diagnostic benchmarks, which simulate perturbations to schemas, questions, and SQL queries to expose brittleness in current models. These studies highlight that relatively small structural or naming changes in database schemas can lead to large drops in accuracy, underscoring the need for approaches that reason explicitly about schema variation rather than relying on fixed training schemas.
